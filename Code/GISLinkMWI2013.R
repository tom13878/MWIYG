...
country.map <- getData('GADM', country=iso3c, level=2)
projection(country.map)

### PREPARE LSMS SPATIAL DATAPOINTS
# Get y2_hhid-GIS link
HH.geo <- read_dta(file.path(dataPath, "Geovariables\\HouseholdGeovariables_IHPS.dta")) %>%
            rename(lat = LAT_DD_MOD, lon = LON_DD_MOD) 

plot.geo <- read_dta(file.path(dataPath, "Geovariables\\PlotGeovariables_IHPS.dta")) 

# There are no ea_id variables in the files
# Create list of coordinates
geo.base <- HH.geo %>% dplyr::select(lat, lon) %>% 
                        unique() %>%
...
# CHECK why projection does not work for map
projection <- "+proj=longlat +datum=WGS84"
country.sp <- spTransform(country.map, CRS(standardproj))

projection(geo.coord)
projection(country.map)
