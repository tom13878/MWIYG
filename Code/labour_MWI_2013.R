# -------------------------------------
#' Household labHHour wave 2 (2013)
...
labHH_RS_prep <- read_dta(file.path(dataPath, "Agriculture/AG_MOD_D.dta")) %>%
  select( y2_hhid, plotnum=ag_d00, ag_d42a:ag_d42p ) %>%
  transmute(y2_hhid, plotnum,
...
labHH_RS_WF <- read_dta(file.path(dataPath, "Agriculture/AG_MOD_D.dta")) %>%
  select( y2_hhid, plotnum=ag_d00, ag_d43a:ag_d43p ) %>%
  transmute(y2_hhid, plotnum,
...
labHH_RS_harv <- read_dta(file.path(dataPath, "Agriculture/AG_MOD_D.dta")) %>%
  select( y2_hhid, plotnum=ag_d00, ag_d44a:ag_d44p ) %>%
  transmute(y2_hhid, plotnum,
...
labHH_RS_prep <- select(labHH_RS_prep,y2_hhid, plotnum,
                        labHH_prep_hrs, labHH_prep_days)
labHH_RS_WF <- select(labHH_RS_WF, y2_hhid, plotnum,
                      labHH_WF_hrs, labHH_WF_days)
labHH_RS_harv <- select(labHH_RS_harv, y2_hhid, plotnum,
...
labHire_RS_non_harv <- read_dta(file.path(dataPath, "Agriculture/AG_MOD_D.dta")) %>%
  select(y2_hhid, plotnum=ag_d00, ag_d47a:ag_d47e ) %>%
  transmute(y2_hhid, plotnum,
...
labHire_RS_harv <- read_dta(file.path(dataPath, "Agriculture/AG_MOD_D.dta")) %>%
  select( y2_hhid, plotnum=ag_d00, ag_d48a:ag_d48e ) %>%
  transmute(y2_hhid, plotnum,
...
labHire_RS_non_harv <- select(labHire_RS_non_harv, y2_hhid, plotnum, labHire_non_harv)
labHire_RS_harv <- select(labHire_RS_harv, y2_hhid, plotnum, labHire_harv)
...
lab_2013 <- transmute(lab, y2_hhid, plotnum,
